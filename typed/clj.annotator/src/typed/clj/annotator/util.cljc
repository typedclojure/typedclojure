   ...
   (let [fv (fn 
         ...
         fvs   (case (:op v)
                 ...
                 :HMap (into []
                             (mapcat fv)
                             (concat
                               (-> v :typed.clj.annotator.rep/HMap-req vals)
                               (-> v :typed.clj.annotator.rep/HMap-opt vals)))
                 :HVec (into []
                             (mapcat fv)
                             (-> v :vec))
                 :union (into []
...
(defn HMap-req-keyset [t]
  {:pre [(r/HMap? t)]
   :post [(set? %)
          (every? keyword? %)]}
  (let [m (map-key-set (:typed.clj.annotator.rep/HMap-req t))]
...
(defn HMap-likely-tag-key 
  ...
  ([hmaps k]
   ...
   (when (every? (fn [m]
                   {:pre [(r/HMap? m)]}
                   (kw-vals? (get (:typed.clj.annotator.rep/HMap-req m) k)))
                 hmaps)
     k)))
