...
(defn default-should-join-HMaps? [t1 t2]
  ...
  (let [ts [t1 t2]
        t1-map (:typed.clj.annotator.rep/HMap-req t1)
        t2-map (:typed.clj.annotator.rep/HMap-req t2)
        res 
        (and
          (<= 2
              (count
...
(defn default-join-HMaps [t1 t2]
  ...
  ;     (unparse-type t1)
  ;     (unparse-type t2))
  (let [t1-req (:typed.clj.annotator.rep/HMap-req t1)
        t2-req (:typed.clj.annotator.rep/HMap-req t2)
        t1-opt (:typed.clj.annotator.rep/HMap-opt t1)
        t2-opt (:typed.clj.annotator.rep/HMap-opt t2)
        all-reqs (set/union
                   (map-key-set t1-req)
                   (map-key-set t2-req))
        ...
        res ;(debug
            ;  (println "Joining HMaps:")
              {:op :HMap
               :typed.clj.annotator.rep/HMap-req (into {}
                                (map (fn [k]
                                       {:pre [(keyword? k)]}
                                       (let [ts (keep k [t1-req t2-req])]
                                         ...
                                         [k (apply join* ts)])))
                                new-reqs)
               :typed.clj.annotator.rep/HMap-opt (into {}
                                (map (fn [k]
                                       {:pre [(keyword? k)]}
                                       (let [ts (keep k [t1-req t2-req
                                                         t1-opt t2-opt])]
  ...
  (let [ts (flatten-unions args)
                         ...
                         (cond
                           ...
                           (do
                             ...
                             #{{:op :HMap
                                ...
                                :typed.clj.annotator.rep/HMap-opt (apply merge-with join (mapcat (juxt :typed.clj.annotator.rep/HMap-req :typed.clj.annotator.rep/HMap-opt) hmaps-merged))}})

                           ;; if one of the common keys is always mapped to a singleton keyword,
                           ;; merge by the value of this key
                           likely-tag
                           (let [by-tag (group-by (fn [m]
                                                    (get (:typed.clj.annotator.rep/HMap-req m) likely-tag))
                                                  hmaps-merged)
                                 new-maps (into #{}
                                                (map merge-HMaps)
                                                (vals by-tag))]
                           ...
                           (let [has-unknown? (atom false)
                                 ...
                                 #{{:op :HMap
                                    ...
                                    :typed.clj.annotator.rep/HMap-req (apply merge-with join
                                                      (map (fn [m]
                                                             {:pre [(HMap? m)]}
                                                             (let [es (select-keys (:typed.clj.annotator.rep/HMap-req m) 
                                                                                   common-keys)]
                                                               (doseq [[_ v] es]
                                                                 (when (unknown? v)
                                    ...
                                    :typed.clj.annotator.rep/HMap-opt (apply merge-with join
                                                      (map (fn [m]
                                                             {:pre [(HMap? m)]}
                                                             (let [es 
                                                                   (merge-with join
                                                                               (:typed.clj.annotator.rep/HMap-opt m)
                                                                               (apply dissoc (:typed.clj.annotator.rep/HMap-req m)
                                                                                      common-keys))]
                                                               (doseq [[_ v] es]
                                                                 (when (unknown? v)
                                                                   (reset! has-unknown? true)))
        ...
        non-hmaps (let [{classes true non-classes false} (group-by -class? non-hmaps)
                        ...
                        classes (let [{seqable-classes true 
                                      ...
                                      (group-by #(contains? relevant-seqables (:typed.clj.annotator.rep/class-instance %)) classes)
                                      seqable-classes
                                      (if (some (comp #{:list :seq :coll}
                                                      :typed.clj.annotator.rep/class-instance)
                                                seqable-classes)
                        ...
                        classes (into #{}
                                      (map (fn [cs]
                                             ...
                                             (-class (-> cs first :typed.clj.annotator.rep/class-instance)
                                                     (apply mapv join* (map :args cs)))))
                                     (vals (group-by :typed.clj.annotator.rep/class-instance classes)))]
        ...
        hmaps-merged (if (some (fn [m]
                                 (and (-class? m)
                                      (#{:map} (:typed.clj.annotator.rep/class-instance m))
        ...
        ts (let [merge-same-length-HVecs (fn [hvs]
                 ...
                 (let [;; at this point, collection classes are normalized to either IPC or IPV.
                       ...
                       (group-by
                         (every-pred
                           -class?
                           (comp boolean #{:vector :coll} :typed.clj.annotator.rep/class-instance))
                         non-HVecs)
                       ...
                       (if (seq vec-classes)
                         [[]
                          (cons 
                            (let [class-name (if (every? (comp boolean #{:vector} :typed.clj.annotator.rep/class-instance) vec-classes)
                                               :vector
                                               :coll)
        ...
        seqable-t? (fn [m]
                     (boolean
                       (when (-class? m)
                         ...
                         (#{:vector :map :coll :seqable} (:typed.clj.annotator.rep/class-instance m)))))
        atomic-type? (fn [v]
                       (boolean
                         (or
                           (and (val? v)
                           ...
                           (and (-class? v)
                                (#{:symbol :string :keyword}
                                  (:typed.clj.annotator.rep/class-instance v))))))
        ]
    ;(prn "union ts" ts)
    (assert (set? ts))
...
(defn join [t1 t2]
  ...
  (let [;id (gensym (apply str (map :op [t1 t2])))
        ...
        res (cond
              ...
              (and (#{:class} (:op t1))
                   (#{:class} (:op t2))
                   (= (:typed.clj.annotator.rep/class-instance t1)
                      (:typed.clj.annotator.rep/class-instance t2))
                   (= (count (:args t1))
                      (count (:args t2))))
              (-class (:typed.clj.annotator.rep/class-instance t1) (mapv join (:args t1) (:args t2)))
