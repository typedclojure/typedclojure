(ns typed.clj.annotator.frontend.spec
  (:require [typed.clj.annotator.util
            ...
            [typed.clj.annotator.join :refer [make-Union join]]
            [typed.clj.annotator.track :refer [local-fn-symbol?]]
            [clojure.core :as core]
            [typed.clj.annotator.rep :as r]
  ))
...
(defn unparse-spec' [{:as m}]
  ...
  (case (:op m)
    ...
    :val (let [t (:val m)]
           (cond
             (:typed.clj.annotator.frontend.spec/implicit-alias m) (unparse-spec (:typed.clj.annotator.frontend.spec/implicit-alias m))
    ...
    :union (if (:typed.clj.annotator.frontend.spec/implicit-alias m)
             (unparse-spec (:typed.clj.annotator.frontend.spec/implicit-alias m))
             (let [env *envs*
                   fully-res (if env
               ...
               (if-let [tag (and env
                 ...
                 (let [multispecs *multispecs-needed*
                       ...
                       tag-for-hmap (fn [t]
                                      {:pre [(r/HMap? t)]}
                                      (let [this-tag (get (:typed.clj.annotator.rep/HMap-req t) tag)
    ...
    :HMap (let [specify-keys 
                  ...
                  (->> entries
                       (map (fn [[k v]]
                              ...
                              (let [a (or (when (r/alias? v) v)
                                          (:typed.clj.annotator.frontend.spec/implicit-alias v))]
                ...
                {req true req-un false} (group-by-qualified (:typed.clj.annotator.rep/HMap-req m))
                {opt true opt-un false} (group-by-qualified (:typed.clj.annotator.rep/HMap-opt m))]
           ...
           (cond
             ...
             (let [;; if we have a macro, ignore the first two arguments
                                               ...
                                               (let [[d] dom]
                                                 (when d
                                                   (and
                                                     (#{:class} (:op d))
                                                     (= :vector
                                                        (:typed.clj.annotator.rep/class-instance d))))))
                                             arities)
                ...
                (list* (qualify-spec-symbol 'fspec)
                       [:args dom-specs
                        :ret rngs]))))
    :class (let [cls (:typed.clj.annotator.rep/class-instance m)
                 args (:args m)]
             (cond
                                                   ...
                                                   (and (#{:class} (:op t))
                                                        (not
                                                          (#{:set
                                                             ...
                                                             :coll
                                                             :seq}
                                                            (:typed.clj.annotator.rep/class-instance t)))))))
                                           alts)]
    ;(prn "or-spec" alts)
    (cond
      (and (seq alts)
...
; For these, add a :typed.clj.annotator.frontend.spec/implicit-alias entry to the :val (or :union, sometimes) map
; that is the alias to use in spec generation.
; Then, need a special case in unparse-type to ensure :typed.clj.annotator.frontend.spec/implicit-alias counts as
; as an alias usage.
...
(defn ensure-alias-for-spec-keys [env config [k t]]
  ...
  (let [a (or (when (r/alias? t) t)
              (:typed.clj.annotator.frontend.spec/implicit-alias t))]
    (if (and (r/alias? a)
      ...
      (let [[sym env] (register-unique-alias-for-spec-keys env config k t)
        ...
        [env (if (kw-vals? t)
               (assoc t :typed.clj.annotator.frontend.spec/implicit-alias new-alias)
               new-alias)]))))
    ...
    (case (:op m)
      ...
      :HMap (let [process-HMap-entries (fn [env es]
                                         ...
                                         (let [[env ts] (accumulate-env implicit-aliases-for-type env config (vals es))
                                               ...
                                               es (zipmap (keys es) ts)]
                                           [env es]))
                  [env req] (process-HMap-entries env (:typed.clj.annotator.rep/HMap-req m))
                  [env opt] (process-HMap-entries env (:typed.clj.annotator.rep/HMap-opt m))]
              [env (assoc m
