...
;; based on clojure.tools.reader.reader-types
(ns ^{:doc "Protocols and default Reader types implementation"
      :author "Bronsa"}
    typed.clj.reader.reader-types
  (:refer-clojure :exclude [char read-line])
  (:require [typed.clj.reader.impl.utils :refer [char whitespace? newline? make-var]])
...
(extend-protocol ReaderCoercer
  ...
  typed.clj.reader.reader_types.Reader
  (to-rdr [rdr] rdr)
  String
...
(extend-protocol PushbackReaderCoercer
  ...
  typed.clj.reader.reader_types.PushbackReader
  (to-pbr [rdr buf-len] (push-back-reader rdr buf-len))
  String
  ...
  (let [frame (.source-log-frames ^SourceLoggingPushbackReader reader)
    ...
    (with-bindings {frame new-frame}
      (let [ret (f)
            update-val-map (fn [m]
                             (cond-> m
                               (instance? clojure.lang.IObj (:val m))
                               (update :val vary-meta assoc
                                       :source (peek-source-log frame))))]
        (cond
          (vector? ret) (let [nforms (count ret)]
                          (cond-> ret
                            (pos? nforms) (update (dec nforms) update-val-map)))
          (map? ret) (update-val-map ret)
          :else ret)))))
...
(defn indexing-reader?
  ...
  [rdr]
  (or (instance? typed.clj.reader.reader_types.IndexingReader rdr)
      (instance? LineNumberingPushbackReader rdr)
      (and (not (instance? typed.clj.reader.reader_types.PushbackReader rdr))
           (not (instance? typed.clj.reader.reader_types.StringReader rdr))
           (not (instance? typed.clj.reader.reader_types.InputStreamReader rdr))
